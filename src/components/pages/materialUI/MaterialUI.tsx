...
import { MaterialUITodoList } from './MaterialUITodoList';
import { v1 } from 'uuid';
...
export const MaterialUI = () => {
  ...
  function removeTask(id: string, todolistId: string) {
    let forRemove = todoFromServer.map((task) =>
      task.todolistId === todolistId
        ? { ...task, tasks: task.tasks.filter((item) => item.taskId !== id) }
        : task
    );
  ...
  function addTask(title: string, todolistId: string) {
    ...
    let newTaskToTodoList = todoFromServer.map((tl) =>
